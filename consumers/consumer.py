...
class KafkaConsumer:
    ...
    def _consume(self):
        ...
        #
        #
        logger.info(f"start consuming messages from {self.topic_name_pattern}")
        try:
            message = self.consumer.poll(timeout=self.consume_timeout)
            if message is None:
                logger.error(f"No messages in topic {self.topic_name_pattern}")
                return 0
            elif message.error() is not None:
                logger.error(f"Messages has errors in {self.topic_name_pattern}")
                return 0
            else:
